...
TEST_CASE( "python3 runtime api", "[python3runtime]" )
{
	...
	SECTION("runtime_test_target.testmap.set_get_contains")
	{
		...
		wrapper[1]->cdt_val.metaffi_string8_val = (char8_t*)u8"key\0";
		...
		wrapper_contains_params[1]->cdt_val.metaffi_string8_val = (char8_t*)u8"key\0";
	...
	SECTION("runtime_test_target.testmap.set_get_contains_cpp_object")
	{
		...
		auto insert = [](void** p_testmap_set, metaffi_handle hthis, const std::u8string& key, std::vector<int>* val, int runtime_id)
		{
		...
		insert(p_testmap_set, testmap_instance, std::u8string(u8"key1"), &vec_to_insert, 733);
		insert(p_testmap_set, testmap_instance, std::u8string(u8"key2"), &vec_to_insert2, 733);
		...
		wrapper_contains_params.set(1, std::u8string(u8"key1"));
		...
		auto get = [&p_testmap_get](metaffi_handle hthis, const std::u8string& key) -> std::vector<int>
		{
			...
			wrapper_get_params[0]->cdt_val.metaffi_handle_val.runtime_id = PYTHON311_RUNTIME_ID;
			wrapper_get_params[1]->type = metaffi_string8_type;
			wrapper_get_params[1]->cdt_val.metaffi_string8_val = (char8_t*)key.c_str();
		...
		std::vector<int> inner_vec1 = get(testmap_instance, std::u8string(u8"key1"));
		REQUIRE(inner_vec1[0] == 1);
		REQUIRE(inner_vec1[1] == 2);
		REQUIRE(inner_vec1[2] == 3);

		std::vector<int> inner_vec2 = get(testmap_instance, std::u8string(u8"key2"));
	...
	SECTION("runtime_test_target.testmap.get_set_name")
	{
		...
		REQUIRE(std::u8string(wrapper_get_ret[0]->cdt_val.metaffi_string8_val) == u8"name1");
		...
		wrapper_set_params[1]->cdt_val.metaffi_string8_val = (char8_t*)u8"name is my name\0";
	...
	SECTION("runtime_test_target.wait_a_bit")
	{
		...
		REQUIRE(getter_ret->pcdt->cdt_val.metaffi_int64_val == 5);
	...
	SECTION("runtime_test_target.ThreeBuffers")
	{
		...
		wrapper_get_ret[0]->cdt_val.metaffi_uint8_array_val.arr[0].length = 2;
		wrapper_get_ret[0]->cdt_val.metaffi_uint8_array_val.arr[0].vals = new metaffi_uint8[2];
		wrapper_get_ret[0]->cdt_val.metaffi_uint8_array_val.arr[0].vals[0] = 1;
		wrapper_get_ret[0]->cdt_val.metaffi_uint8_array_val.arr[0].vals[1] = 2;
		...
		wrapper_get_ret[0]->cdt_val.metaffi_uint8_array_val.arr[2].length = 4;
		wrapper_get_ret[0]->cdt_val.metaffi_uint8_array_val.arr[2].vals = new metaffi_uint8[4];
		...
		wrapper_get_ret[0]->cdt_val.metaffi_uint8_array_val.arr[2].vals[3] = 9;
		...
		((void(*)(void*,cdts*,char**,uint64_t*))pgetThreeBuffers[0])(pgetThreeBuffers[1], get_three_buffers_ret, &err, &long_err_len);
		if(err){ FAIL(err); }
		REQUIRE(long_err_len == 0);

		metaffi::runtime::cdts_wrapper wrapper_get_buffers(get_three_buffers_ret[1].pcdt, get_three_buffers_ret[1].len, false);
	...
	SECTION("runtime_test_target.set_five_seconds")
	{
		...
		set_wrapper[0]->cdt_val.metaffi_int64_val = 10;
		...
		REQUIRE(getter_ret->pcdt->type == metaffi_int64_type);
		REQUIRE(getter_ret->pcdt->cdt_val.metaffi_int64_val == 10);
